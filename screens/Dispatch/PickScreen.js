...
const PickScreen = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log('PickScreen - Initial scan data to API:', data);
      console.log('PickScreen - Initial page parameter:', data.page);
  ...
  // Debug logging to track screenDetail changes
  useEffect(() => {
    if (!isFocused) return;
    
    if (screenDetail) {
      console.log('🔍 PickScreen - screenDetail received:', {
        action: screenDetail.action,
        page: screenDetail.page,
        param: screenDetail.param,
        ref: screenDetail.ref,
        barcode: screenDetail.barcode,
      });
      
      // Note: Label printing is handled by DispatchDetailScreen on action complete
      // PickScreen only handles the picking flow
    }
  }, [screenDetail, isFocused]);
  ...
  const onMessage = event => {
    ...
    console.log('PickScreen - Sending scan data to API:', data);
    console.log('PickScreen - Page parameter:', data.page);
    console.log('PickScreen - Barcode scanned:', data.barcode);
...
`;
  // Check if extraInfo is empty or missing
  const hasContent = screenDetail && screenDetail.extraInfo && screenDetail.extraInfo.trim() !== '';
    ...
    <>
      {errorText ? (
        ...
        <View style={styles.container}>
          <Text style={styles.text}>Scan an Order</Text>
        </View>
      ) : !hasContent ? (
        <View style={styles.container}>
          <Text style={styles.text}>No content available</Text>
          <Text style={styles.subText}>Order: {screenDetail.barcode || 'Unknown'}</Text>
          <Text style={styles.subText}>Status: {screenDetail.action || 'Unknown'}</Text>
        </View>
      ) : (
        <WebView
          ref={webViewRef}
