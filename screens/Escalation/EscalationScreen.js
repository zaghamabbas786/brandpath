  ...
  const onMessage = event => {
    ...
    if (typeof orderInfo === 'string') {
      try {
        ...
        orderInfo = {}; // Fallback to an empty object
      }
    }

    if (code === 'CMD.PRINT.REPRINT_LABEL' || code === 'CMD.PRINT.NEW_LABEL') {
      const payload = {
        InvoiceNum: orderInfo.invoiceNum, // Now this should work
        OrderRef: orderInfo.Ref, // Now this should work
        User: user.username,
